...
namespace Signar.Controllers
{
    ...
    {
        private const int AvatarStoredWidth = 100;
        private const int AvatarStoredHeight = 100;
        ...
        [HttpPost]
        public ActionResult Save(string t, string l, string h, string w, string fileName, int UserID)
        {
            try
            {
                ...
                Stream s = new MemoryStream(img.GetBytes());
                using (UserService userService = new UserService())
                {
                    UserDTO user = userService.GetItem(UserID);
                    userService.EditUserPhoto(user, img.FileName, s);
                }
